...
void SpecificWorker::search_state(const RoboCompYoloObjects::TObjects &objects){
    ...
    if(!robot.has_target())
        for (auto obj : objects)
            if (obj.type != robot.get_current_target().type){
    ...
    robot.set_pure_rotation(0.45f);  
    
}
void SpecificWorker::approach_state(const RoboCompYoloObjects::TObjects &objects, const std::vector<Eigen::Vector2f> &line){
    
    std::cout << "Approaching " << robot.get_current_target().type << std::endl;
    printf("Distance %f\n", robot.get_distance_to_target());

    if (robot.get_distance_to_target() < 800.f){
